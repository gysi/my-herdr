...
directory (see `fork_request`), because herdr actions take no arguments.
"""
from .. import fork, fork_request, herdr
...
def main(args):
    ...
    if request:
        pane_id, name = request["pane_id"], request["name"]
    else:
        pane_id, name = herdr.focused_pane(ctx), None

    forked = fork.fork_into_new_tab(pane_id, name=name)
