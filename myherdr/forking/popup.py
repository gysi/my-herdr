...
`request.py`) and invokes the ordinary `fork-tab` action. `plugin action
...
from . import line_editor, request
from .tab_ask import SOURCE_ENV
...
def request_fork(ctx, pane_id, name):
    ...
    """
    request.save(ctx.state_dir, pane_id, name)
    try:
